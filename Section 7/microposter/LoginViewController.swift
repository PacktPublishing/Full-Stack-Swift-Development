...
class LoginViewController: UIViewController {
    ...
    func loginUser(username: String, password: String) {
        ...
        if let url = URL(string:"http://micropostr-app.herokuapp.com/users/login") {
            ...
            if let loginData = loginString.data(using: .utf8) {
                ...
                let task = session.dataTask(with: request) { (data, response, error) in
                    ...
                    if let resData = data {
                        do {
                            if let jsonData = try JSONSerialization.jsonObject(with: resData, options: []) as? [String: Any] {
                                if let token = jsonData["token"] as? String {
                                    
                                    print(token)
                                    UserDefaults.standard.set(token, forKey: "token")
                                    DispatchQueue.main.async {
                                        SwitchUtility.updateVC()
                                    }
